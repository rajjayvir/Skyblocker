...
public class FancyStatusBars {
    ...
    public boolean render(DrawContext context, int scaledWidth, int scaledHeight) {
        ...
        if (!getConfig.get().general.bars.enableBars || player == null || Utils.isInTheRift())
            return false;
        ...
        for (int i = 0; i < 4; i++) {
            int configAnchorNum = switch (i) {
                case 0 -> getConfig.get().general.bars.barpositions.healthBarPosition.toInt();
                case 1 -> getConfig.get().general.bars.barpositions.manaBarPosition.toInt();
                case 2 -> getConfig.get().general.bars.barpositions.defenceBarPosition.toInt();
                case 3 -> getConfig.get().general.bars.barpositions.experienceBarPosition.toInt();
                default -> 0;
            };
